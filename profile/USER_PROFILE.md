# User Profile

This file pre-loads `/plan-trip` with stable defaults so Claude stops asking the same questions every trip. Every field is **optional** — leave blank to have Claude ask as normal. Profile values are treated as **defaults to confirm**, not silent assumptions: you will still see every value in the Phase 1 confirmation summary and can override for any individual trip.

Edit values below. Remove or leave blank anything you don't want pre-filled. Commented-out lines (`<!-- ... -->`) are examples.

---

## Travelers

- **Default traveler count:**
- **Names / ages:** <!-- e.g. Roald (38), Partner (36) -->
- **Pets:** <!-- none / dog named X / cat named Y -->

## Origin

- **Home address:** <!-- e.g. Rue Example 12, 1955 Chamoson, Switzerland -->
- **Home lat/lng:** <!-- e.g. 46.2017, 7.2260 -->
- **Nearest airports:** <!-- e.g. GVA (Geneva), ZRH (Zurich) -->

## Style & Preferences

- **Vacation style:** <!-- relaxed / balanced / packed / adventure -->
- **Pace:** <!-- e.g. slow mornings, no activities before 10:00 -->
- **Accommodation style:** <!-- budget / mid-range / luxury / mix -->
- **Interests:** <!-- e.g. food & dining, nature & outdoors, culture & history, adventure & sports -->

## Transport

- **Default transport mode:** <!-- own car / rental car / public transit / walking + transit / mix -->
- **Own vehicle:** <!-- make/model, fuel type, EV charging needs -->
- **Driver's license countries:** <!-- e.g. CH, EU -->

## Budget

- **Home currency:** <!-- e.g. CHF, EUR, USD -->
- **Typical tier:** <!-- €, €€, €€€ -->
- **Typical per-person-per-day range:** <!-- e.g. 150-200 CHF -->

## Special Needs

- **Dietary restrictions:** <!-- e.g. vegetarian, gluten-free, none -->
- **Accessibility needs:** <!-- e.g. step-free access, none -->
- **Language preferences:** <!-- e.g. English, French, German -->

---

## How this is used

When you run `/plan-trip`, Phase 0 reads this file and pre-fills Phase 1 requirements. In the confirmation summary, profile-sourced values are tagged `(from profile)` so you can see exactly what was loaded vs. what you provided for this trip. Override any value per-trip — the profile file is **not** auto-updated.

To disable for a session, rename the file or remove its fields.
